...
class SSP {
	...
	static function data_output ( $columns, $data )
	{
		...
		if (mysqli_num_rows($res) > 0) {
			while ($row = mysqli_fetch_assoc($res)) {
				$data_arr[] = array("id" => $row["id"], "name" => $row["name"], "pid" => $row["pid"], "desc" => $row["description"], "is_active" => $row["is_active"]);
			}
		}

		$data_ar = array_values(self::buildTree($data_arr));
	...
	static function buildTree($array, $id_key = "id", $parent_key = "pid") {
        $res = [];
        foreach ($array as $y) {
        ...
        foreach ($array_with_id as $key => $element) {
            if ($element["pid"]) {
                // $array_with_id[$element[$parent_key]]['children'][$key] = &$array_with_id[$key];
