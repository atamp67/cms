...
class Post {	
	...
	// private $postTable = 'posts';
	// private $categoryTable = 'post_category';
	...
	public function getPostsListing(){		
		...
		$sqlQuery = "
			SELECT p.id, p.title, u.first_name, u.last_name, p.status, p.created_date, p.updated_at
		...
		while ($post = $result->fetch_assoc()) { 				
			...
			$rows[] = ucfirst($post['id']);
			$rows[] = $post['title'];	
			$rows[] = ucfirst($post['first_name'])." ".$post['last_name'];	
			$rows[] = $status;				
			$rows[] = $post['created_date'];	
			$rows[] = $post['updated_at'];
			$rows[] = '<a href="update_post.php?id='.$post["id"].'" class="btn btn-warning btn-xs update">Edit</a>' . ' &nbsp;<button type="button" name="delete" id="'.$post["id"].'" class="btn btn-danger btn-xs delete" >Delete</button>';
			$posts[] = $rows;
		}
